...
// Ex1 
// 1.1 Quelle est la variable partagee : somme_
// 1.2 Section critique : la for loop qui touche a somme_
// 1.3 Mettre en place une solution 
// Solutions possibles :1)lock et unlock , 2) lock guard

// Espace anonyme pour les variables globales et partagées :
namespace {
    int somme_;     // Contiendra la somme (devrait être 50005000).
    std::mutex somme_mutex;
}

void accum(int d, int f)
{
    const std::lock_guard<std::mutex> lock(somme_mutex);
