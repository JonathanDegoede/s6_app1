...
namespace gif643
{
    ...
    using TaskHashMap = std::unordered_map<std::string, bool>;
    ...
    public:
        ...
        Processor(int n_threads) : should_run_(true)
        {
            if (n_threads <= 0)
            {
...
int main(int argc, char **argv)
{
    ...
    while (!std::cin.eof())
    {
        ...
        if (!line.empty())
        {
            TaskHashMap::const_iterator iterator = task_cache.find(line);
            if (iterator != task_cache.end()){
                std::cerr<<"This request has already been handled: " << line <<std::endl;
                continue;
            }
            proc.parseAndQueue(line);
        }
        task_cache[line] = true; 
    }
